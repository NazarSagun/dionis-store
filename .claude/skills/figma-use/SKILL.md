...
> Vendored from the Figma MCP server's `skill://figma/figma-use/SKILL.md` resource for offline/repo reference. Using `use_figma` itself requires the Figma MCP connector to be connected — this file alone does not add that tool. The `references/*.md` and `plugin-api-standalone.d.ts` files linked below live inside the live MCP skill tree, not in this repo; fetch them live via the Figma MCP's `get_figma_skill` tool (`skill://figma/figma-use/references/<path>`) when needed.
