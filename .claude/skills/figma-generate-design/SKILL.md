...
> Vendored from the Figma MCP server's `skill://figma/figma-generate-design/SKILL.md` resource for offline/repo reference. This skill's `use_figma` / `get_screenshot` / `search_design_system` etc. calls require the Figma MCP connector to actually be connected — this file alone does not add those tools. Links below to sibling skills (`figma-use`, `figma-generate-library`, `figma-code-connect`) and `references/*.md` files point at paths inside the live MCP skill tree, not files in this repo; fetch them live via the Figma MCP's `get_figma_skill` tool (`skill://figma/<name>/...`) when needed.
