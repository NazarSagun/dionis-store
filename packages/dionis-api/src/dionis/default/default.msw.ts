...
export const getGetOrdersMockHandler = (overrideResponse?: OrdersPage | ((info: Parameters<Parameters<typeof http.get>[1]>[0]) => Promise<OrdersPage> | OrdersPage)) => {
...
export const getGetOwnedItemsMockHandler = (overrideResponse?: OwnedItem[] | ((info: Parameters<Parameters<typeof http.get>[1]>[0]) => Promise<OwnedItem[]> | OwnedItem[])) => {
  return http.get('*/orders/owned', async (info) => {await delay(1000);
    return new HttpResponse(JSON.stringify(overrideResponse !== undefined 
            ? (typeof overrideResponse === "function" ? await overrideResponse(info) : overrideResponse) 
            : getGetOwnedItemsResponseMock()),
      {
        status: 200,
        headers: {
          'Content-Type': 'application/json',
